...
def main():
    ...
    be sure that you have (1) images in ./img folder, (2) attentive verified the
    necessary configurations in `scripts/helpful_scripts.py` and
    after that ran this commando:

        #Don't forget to specify a wishful network (`development` by default).

        $ brownie run scripts/1_deploy_nft.py
